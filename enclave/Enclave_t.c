...
static sgx_status_t SGX_CDECL sgx_seal(void* pms)
{
	...
	ms->ms_retval = seal(_in_blob, _tmp_len);
	if (_in_blob) {
		if (memcpy_s(_tmp_blob, _len_blob, _in_blob, _len_blob)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_unseal(void* pms)
{
	...
	ms->ms_retval = unseal(_in_blob, _tmp_len);
	if (_in_blob) {
		if (memcpy_s(_tmp_blob, _len_blob, _in_blob, _len_blob)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
...
sgx_status_t SGX_CDECL save_to_db(sgx_status_t* retval, uint8_t* key_pointer, uint32_t key_size, uint32_t sealed_log_size, uint8_t* scratch_pad_pointer)
{
	...
	size_t _len_scratch_pad_pointer = sealed_log_size;
	...
	if (key_pointer != NULL) {
		ms->ms_key_pointer = (uint8_t*)__tmp;
		if (_len_key_pointer % sizeof(*key_pointer) != 0) {
			sgx_ocfree();
			return SGX_ERROR_INVALID_PARAMETER;
		}
		if (memcpy_s(__tmp, ocalloc_size, key_pointer, _len_key_pointer)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
	...
	if (scratch_pad_pointer != NULL) {
		ms->ms_scratch_pad_pointer = (uint8_t*)__tmp;
		if (_len_scratch_pad_pointer % sizeof(*scratch_pad_pointer) != 0) {
			sgx_ocfree();
			return SGX_ERROR_INVALID_PARAMETER;
		}
		if (memcpy_s(__tmp, ocalloc_size, scratch_pad_pointer, _len_scratch_pad_pointer)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
...
sgx_status_t SGX_CDECL get_from_db(sgx_status_t* retval, uint8_t* key_pointer, uint32_t key_size, uint8_t* value_pointer, uint32_t value_size)
{
	...
	if (value_pointer != NULL) {
		ms->ms_value_pointer = (uint8_t*)__tmp;
		__tmp_value_pointer = __tmp;
		if (_len_value_pointer % sizeof(*value_pointer) != 0) {
			sgx_ocfree();
			return SGX_ERROR_INVALID_PARAMETER;
		}
		memset(__tmp_value_pointer, 0, _len_value_pointer);
